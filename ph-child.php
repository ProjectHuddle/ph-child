...
if ( ! class_exists( 'PH_Child' ) ) :
	...
	final class PH_Child {
		...
		public function enqueue_admin_scripts() {
			...
			if ( $screen->id !== 'settings_page_feedback-connection-options' ) {
				return;
			}

			// Check if built assets exist
			$js_file = PH_CHILD_PLUGIN_DIR . 'assets/dist/surefeedback-admin.umd.js';
			$css_file = PH_CHILD_PLUGIN_DIR . 'assets/dist/surefeedback-admin.css';

			if ( file_exists( $js_file ) ) {
				wp_enqueue_script(
					'surefeedback-admin',
					PH_CHILD_PLUGIN_URL . 'assets/dist/surefeedback-admin.umd.js',
					array(),
					filemtime( $js_file ),
					true
				);
			...
			if ( file_exists( $css_file ) ) {
				wp_enqueue_style(
					'surefeedback-admin',
					PH_CHILD_PLUGIN_URL . 'assets/dist/surefeedback-admin.css',
					array(),
					filemtime( $css_file )
				);
			}
